...
public class Classic_ExistingUserUsing3DVisaInUK extends spafinder.com.base.UKBaseClass  {
	...
	@Test (enabled = true)
	public void ClassicMemberUsing3dVisa_InUK () throws InterruptedException, IOException{
...
// Billing page
	    ...
	    if (PF == false)
	    {
	    	passData ("Order", "passFail", 14, cv.Fail); // writing the text into excel from selenium
	    } else  { 
	    	passData ("UK", "passFail", 14, cv.Pass); // writing the text into excel from selenium
	    	setCellData ("UK", "OrderNumber", 14, cl.orderNumber); // Writing UK number into excel file.
	    	System.out.println(getCellData("UK", "OrderNumber", 14)); // reading from excel\

	    	passData ("UK", "Total", 14, totalAmount);
