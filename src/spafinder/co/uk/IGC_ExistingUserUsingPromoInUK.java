...
public class IGC_ExistingUserUsingPromoInUK extends spafinder.com.base.UKBaseClass {
	IGC_Locator gcl = new IGC_Locator ();
	IGC_Values  gcv = new IGC_Values ();
	
	
@Test
public void IGC_ExistingUserWithPromoInUK () throws InterruptedException, IOException{
...
// Regular Expression 
    ...
    boolean PF = driver.findElement(By.xpath(gcl.orderNumber)).isDisplayed();
 	if (PF == false)
 	 {
 		passData ("UK", "passFail", 5, gcv.Fail); // writing the text into excel from selenium
 	} else  { 
 		passData ("UK", "passFail", 5, gcv.Pass); // writing the text into excel from selenium
 		setCellData ("UK", "OrderNumber", 5, gcl.orderNumber); // Writing order number into excel file.
 		System.out.println(getCellData("UK", "OrderNumber", 5)); // reading from excel\

 		passData ("UK", "Total", 5, promoTotal);
