...
public class Classic_GuestUsingFlyerMilesInUSA extends spafinder.com.base.USABaseClass  {
...
@Test
public void ClassicGuestUsingFlyerMilesInUSA () throws InterruptedException, IOException{
...
// Confirmation Page
	   ...
	   boolean PF = driver.findElements(By.xpath(cl.orderNumber)).size() != 0;
		if (!PF)
		 {
			passData ("USA", "passFail", 19, cv.Fail); // writing the text into excel from selenium
		 } else  { 
			 passData ("USA", "passFail", 19,cv.Pass); // writing the text into excel from selenium
			 setCellData ("USA", "OrderNumber", 19, cl.orderNumber); // Writing USA number into excel file.
			 System.out.println(getCellData("USA", "OrderNumber", 19)); // reading from excel\

			 passData ("USA", "Total", 19, promoTotal);
			 System.out.println ("Total Price is: " + promoTotal);
			 System.err.println ("=====================================================================");
         
		 		}
			}
	    screenShot ("FlyerMiles_ClassicGuest_USA");
	       }
